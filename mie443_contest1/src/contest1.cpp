...
int main(int argc, char **argv)
{
    ...
    uint8_t prev_process_step = 1;
    float sweeping_direction = 1.0;
    float default_linear = 0.5;
    float default_angular = M_PI/4.0;
    float prev_yaw = 0.0;
    float correct_yaw = 0.0;
    float start_yaw = 0.0;
    ...
    while(ros::ok() && secondsElapsed <= 480) {
       ...
       for (uint32_t b_idx = 0; b_idx < N_BUMPER; ++b_idx) {
        ...
        secondsElapsed = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now()-start).count();
        loop_rate.sleep();
        ROS_INFO("Process Step: %i",process_step);
        ...
        else if (process_step == 9){
            if ((yaw-correct_yaw) <= 0.1){
                vel.angular.z = 0;
                vel.linear.x = 0;
                vel_pub.publish(vel);
                process_step = prev_process_step;
            }
            else {
                if (yaw-correct_yaw > 0){
                    vel.angular.z = -default_angular;
                }
                else {
                    vel.angular.z = default_angular;
                }
        ...
        if (any_bumper_pressed){
            ...
            vel.linear.x = 0;
            vel_pub.publish(vel);
            prev_process_step = process_step+1;
            correct_yaw = prev_yaw;
            process_step = 8;
            ROS_INFO("I have hit a bumper");
        }
        ...
        ROS_INFO("velocity is: %f, %f", vel.linear.x, vel.angular.z);
